...
extension UIAlertController {
    ...
    ///
    /// - Parameters:
    ///   - viewController: from view controller otherwise use window rootViewController
    ///   - animated: set true to animate presentation of alert controller (default is true).
    ///   - completion: an optional completion handler to be called after presenting alert controller (default is nil).
    public func show(from viewController: UIViewController? = nil, animated: Bool = true, completion: (() -> Void)? = nil) {
        ...
        DispatchQueue.main.async {
            fromViewController?.present(self, animated: animated, completion: completion)
        }
    }
    ...
    func addAction(image: UIImage? = nil, title: String, color: UIColor? = nil, style: UIAlertAction.Style = .default, isEnabled: Bool = true, isSelected: Bool = false, handler: ((UIAlertAction) -> Void)? = nil) {
        //let isPad: Bool = UIDevice.current.userInterfaceIdiom == .pad
